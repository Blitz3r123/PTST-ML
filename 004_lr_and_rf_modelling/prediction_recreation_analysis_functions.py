...
BEST_LR_INTERPOLATION_MODELS_PATH = "best_interpolation_models_lr_models.csv"
BEST_RF_INTERPOLATION_MODELS_PATH = "best_interpolation_models_rf_models.csv"

BEST_LR_EXTRAPOLATION_MODELS_PATH = "best_extrapolation_models_lr_models.csv"
BEST_RF_EXTRAPOLATION_MODELS_PATH = "best_extrapolation_models_rf_models.csv"

all_model_paths = [
    BEST_LR_INTERPOLATION_MODELS_PATH, 
    BEST_RF_INTERPOLATION_MODELS_PATH, 
    BEST_LR_EXTRAPOLATION_MODELS_PATH, 
    BEST_RF_EXTRAPOLATION_MODELS_PATH
]
...
lr_interpolation_models = pd.read_csv(BEST_LR_INTERPOLATION_MODELS_PATH)
rf_interpolation_models = pd.read_csv(BEST_RF_INTERPOLATION_MODELS_PATH)
lr_extrapolation_models = pd.read_csv(BEST_LR_EXTRAPOLATION_MODELS_PATH)
rf_extrapolation_models = pd.read_csv(BEST_RF_EXTRAPOLATION_MODELS_PATH)
...
def get_model_info_from_metric(model_type, is_interpolation, metric):
    if model_type is None or metric is None or is_interpolation is None:
        print("Missing arguments")
        return None
    ...
    if metric not in METRICS:
        ...
        return None
    if is_interpolation not in [True, False]:
        print(f"Unknown is_interpolation: {is_interpolation}")
        return None
    
    if model_type == 'lr':
        if is_interpolation:
            row = lr_interpolation_models[lr_interpolation_models['metric'] == metric]
        else:
            row = lr_extrapolation_models[lr_extrapolation_models['metric'] == metric]
    elif model_type == 'rf':
        if is_interpolation:
            row = rf_interpolation_models[rf_interpolation_models['metric'] == metric]
        else:
            row = rf_extrapolation_models[rf_extrapolation_models['metric'] == metric]
...
def predict_random_test_for_metric(model_json, model, metric, test_count):
    ...
    for i in range(row_count * row_count):
        ...
        ax.scatter(y_test.values[random_index], y_test_pred[random_index], s=3, label=f"Test RMSE: \n{avg_test_rmse:,.2f}\nTest R2: \n{avg_test_r2:.2f}")
